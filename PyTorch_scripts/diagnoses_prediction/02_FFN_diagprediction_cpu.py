...
def train():
    ...
    for ind_i in range(0,k):
        ...
        tensor_x = torch.Tensor(arrX).to(device)
        numplist = np.array(Y_train)
        arrY = np.concatenate(numplist).tolist()
        tensor_y = torch.Tensor(arrY).to(device)
        print("Shape X:", np.shape(arrX), "Shape Y:", np.shape(arrY))
        # tensor_x = torch.Tensor(np.array(X_train).tolist()).to(device)  # transform to torch tensor
        # tensor_y = torch.Tensor(np.array(Y_train).tolist()).to(device)
        ...
        # Test loader
        tensor_x = torch.Tensor(np.array(X_test).tolist()).to(device)  # transform to torch tensor
        tensor_y = torch.Tensor(np.array(Y_test).tolist()).to(device)
        dataset = dt.TensorDataset(tensor_x, tensor_y)  # create your dataset
