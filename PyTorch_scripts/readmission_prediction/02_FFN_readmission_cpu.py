...
def train():
    ...
    for ind_i in range(0, k):
        ...
        # Data loader0
        tensor_x = torch.ByteTensor(np.concatenate([np.array(plist, dtype=np.uint8) for plist in X_train]).tolist()).cpu()
        tensor_y = torch.ByteTensor(np.concatenate([np.array(plist, dtype=np.uint8) for plist in Y_train]).tolist()).cpu()
